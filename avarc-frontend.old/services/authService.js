...
const getCurrentUser = async () => {
    try {
        ...
        if (!response.data) {
            console.error('[authService] No data in getCurrentUser response');
            throw new Error('Invalid response from server');
        }

        // Store the UUID in localStorage if it's not already there
        if (response.data.uuid && !localStorage.getItem('uuid')) {
...
const updateUser = async (uuid, updateData) => {
    try {
        const response = await apiClient.put(`${ENDPOINTS.USERS.BASE}/${uuid}`, updateData);
        return {
            success: true,
            data: response.data
        };
        ...
        return {
            success: false,
            error: error.response?.data?.errorMessage || 'Failed to update user'
        };
    }
};
